...
//显示器
constexpr char pageMain[pM_height][pM_width] = {"========================\0",
                                                "请选择：\0",
                                                "0.关闭电机\0",
                                                "1.开启电机\0",
                                                "2.修改电机速度\0",
...
Motor::PARAM Motor::getMotorParam() { return motor; }

void Motor::setMotorParam(PARAM m) { motor = m; }
...
void MotorController::printer() {
    // printf("\n");
    for (int i = 0; i < pM_height; i++) printf("%s\n", pageMain[i]);
}
...
bool MotorController::setRot() {
    ...
    switch (a) {
        case 0:
            cmotor.rot = ROT_CLOCKWISE;
            break;
        case 1:
            cmotor.rot = ROT_COUNTER;
            break;
        default:
            printf("\n你可能不能这么选择\n");
            SLEEP;  //阻塞线程
